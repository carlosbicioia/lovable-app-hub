...
export default function CalendarView() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5rem)]">
      ...
      {/* Operator legend chips – always visible in week view */}
      {view === "week" && (
        <div className="flex flex-wrap items-center gap-1.5 shrink-0 mt-2">
          ...
          {allOps.map((op) => (
            <button
              ...
              style={{
                backgroundColor: `hsl(${op.color} / 0.12)`,
                color: `hsl(${op.color})`,
                borderColor: `hsl(${op.color} / 0.3)`,
              }}
            >
              <span className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ backgroundColor: `hsl(${op.color})` }} />
              {op.name.split(" ")[0]}
              <span className={cn("text-[10px] px-1 py-0 rounded-full border", specialtyColor[op.specialty])}>
                {specialtyIcon[op.specialty]}
              </span>
            </button>
          ))}
