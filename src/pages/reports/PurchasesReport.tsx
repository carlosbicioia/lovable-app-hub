...
export default function PurchasesReport() {
  ...
  const stats = useMemo(() => {
    ...
    const topSuppliersByAmount = Object.entries(supplierMap).sort((a, b) => b[1].total - a[1].total).slice(0, 10)
      .map(([name, v]) => ({ name, value: v.total }));
    const topSuppliersByCount = Object.entries(supplierMap).sort((a, b) => b[1].count - a[1].count).slice(0, 10)
    ...
    const exported = invoices.filter((i) => i.status === "Exportada").reduce((a, i) => a + i.total, 0);

    return { total, totalAmount, avgAmount, totalTax, byStatus, topSuppliersByAmount, topSuppliersByCount, monthly, pending, validated, exported, supplierMap };
  }, [invoices]);
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={() => {
            const rows = Object.entries(stats.supplierMap).sort((a, b) => b[1].total - a[1].total)
              .map(([name, v]) => [name, String(v.count), v.total.toFixed(2), (v.total / v.count).toFixed(2)]);
