...
BEGIN
  ...
  -- Delete purchase invoice lines linked to purchase orders of this service
  DELETE FROM public.purchase_invoice_lines WHERE purchase_order_id IN (
    SELECT id FROM public.purchase_orders WHERE service_id = OLD.id
  );
  -- Also delete purchase invoice lines linked directly to service
