...
import { Users, Wrench, AlertTriangle, TrendingUp, Clock, Handshake, Star, Euro, ShoppingCart, FileText, UserCheck } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <KpiCard title="Servicios" value={totalServices} subtitle="Totales dados de alta" icon={Wrench} variant="primary" onClick={() => router.push("/servicios")} />
        <KpiCard title="Pte. Contacto" value={pendingContact} subtitle="Sin contactar aún" icon={Clock} variant="warning" onClick={() => router.push("/servicios?status=Pendiente_Contacto")} />
        <KpiCard title="Asignado" value={assigned} subtitle="Con técnico, sin cita" icon={UserCheck} variant="default" onClick={() => router.push("/servicios?status=Asignado")} />
        <KpiCard title="En Curso" value={inProgress} subtitle="Técnicos trabajando" icon={TrendingUp} variant="info" onClick={() => router.push("/servicios?status=En_Curso")} />
        <KpiCard title="Urgencias" value={urgent} subtitle="No cerradas" icon={AlertTriangle} variant="warning" onClick={() => router.push("/servicios?urgency=urgent")} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <KpiCard title="Clientes" value={uniqueClients} icon={Users} variant="default" onClick={() => router.push("/clientes")} />
        <KpiCard title="Colaboradores" value={uniqueCollabs} icon={Handshake} variant="success" onClick={() => router.push("/colaboradores")} />
        ...
        <KpiCard
          ...
          icon={FileText}
          variant="warning"
          onClick={() => router.push("/servicios?status=Finalizado")}
        />
      </div>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.slice(0, 10).map((s) => (
                  <tr key={s.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => router.push(`/servicios/${s.id}`)}>
