...
export default function BudgetCreate() {
  ...
  const addLineFromArticle = (articleId: string) => {
    ...
    if (openPopoverIndex !== null && openPopoverIndex < lines.length) {
      ...
      setLines((prev) => [
        ...prev,
        { id: String(prev.length + 1), concept: article.title, description: article.description, units: 1, costPrice: article.costPrice, margin: Math.round(margin * 100) / 100, taxRate: 21 },
      ]);
    }
  ...
  const handleSave = async (send: boolean) => {
    ...
    if (send && lines.length === 1 && !lines[0].concept.trim()) { toast.error("Añade al menos una línea con contenido antes de enviar"); return; }
  ...
  return (
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => router.push("/presupuestos")}>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Popover open={serviceSearchOpen} onOpenChange={setServiceSearchOpen}>
                <PopoverTrigger asChild>
                  <Button variant="outline" role="combobox" aria-expanded={serviceSearchOpen} className="w-full justify-between font-normal">
                    {selectedService ? `${selectedService.id} — ${selectedService.clientName}` : "Buscar servicio..."}
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {servicesWithBudget.map((s) => (
                          <CommandItem key={s.id} value={`${s.id} ${s.clientName} ${s.address ?? ""} ${s.collaboratorName ?? ""}`} onSelect={() => { setServiceId(s.id); setServiceSearchOpen(false); }}>
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-muted-foreground">{s.address || "Sin dirección"}{s.collaboratorName ? ` · ${s.collaboratorName}` : ""}</span>
                            </div>
                          </CommandItem>
                        ))}
                      </CommandGroup>
            ...
            {selectedService && (
              <>
                <div className="space-y-2"><Label>Cliente</Label><Input value={selectedService.clientName} readOnly className="bg-muted" /></div>
                <div className="space-y-2"><Label>Dirección</Label><Input value={selectedService.address ?? "—"} readOnly className="bg-muted" /></div>
                <div className="space-y-2"><Label>Colaborador</Label><Input value={selectedService.collaboratorName ?? "Sin colaborador"} readOnly className="bg-muted" /></div>
              </>
            )}
          </div>
        </CardContent>
      ...
      <div className="flex justify-end gap-3">
        <Button variant="outline" onClick={() => handleSave(false)}><Save className="w-4 h-4 mr-2" /> Guardar borrador</Button>
        <Button onClick={() => handleSave(true)}><Send className="w-4 h-4 mr-2" /> Guardar y enviar por email</Button>
      </div>
    </div>
    </>
  );
