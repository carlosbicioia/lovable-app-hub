  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => router.push("/presupuestos")}><ArrowLeft className="w-5 h-5" /></Button>
        <div className="flex-1 flex items-center gap-3">
          <h1 className="text-2xl font-display font-bold text-foreground">{budget.id}</h1>
          <span className={cn("inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium", cfg.className)}>{cfg.label}</span>
        </div>
        <Button variant="outline" onClick={() => {
          generateDocumentPdf({
            ...
            subtotal,
            taxBreakdown: Object.entries(taxGroups).map(([rate, amount]) => ({ rate: Number(rate), amount: amount as number })),
            total,
        ...
        <Button onClick={handleSendEmail}><Send className="w-4 h-4 mr-2" /> Enviar por email</Button>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm p-8 max-w-4xl mx-auto print:shadow-none print:border-0 print:p-0">
        <div className="flex justify-between items-start mb-8">
          <CompanyLogo size="lg" className="rounded-lg" fallback={
            <h2 className="text-3xl font-display font-bold tracking-tight text-foreground">URBAN<span className="text-primary">GO</span></h2>
          } />
        ...
        <div className="flex justify-between mb-6">
          ...
          <p className="text-sm text-muted-foreground">Fecha: {format(new Date(budget.createdAt), "dd/MM/yyyy", { locale: es })}</p>
        </div>
        ...
        <div className="flex justify-end">
          <div className="w-64 space-y-1 text-sm">
            <div className="flex justify-between"><span className="font-medium text-card-foreground">Subtotal:</span><span className="text-card-foreground font-medium">{subtotal.toFixed(2)} €</span></div>
            ...
            <div className="flex justify-between border-t border-foreground pt-1"><span className="font-bold text-card-foreground">TOTAL:</span><span className="font-bold text-card-foreground">{total.toFixed(2)} €</span></div>
            {hasCommission && (
              <>
                <div className="flex justify-between pt-1"><span className="text-muted-foreground">Comisión {budget.collaboratorName} ({commissionRate}%)</span><span className="text-muted-foreground">-{(total * commissionRate / 100).toFixed(2)} €</span></div>
                <div className="flex justify-between border-t border-border pt-1"><span className="font-semibold text-card-foreground">Neto tras comisión:</span><span className="font-semibold text-card-foreground">{(total - total * commissionRate / 100).toFixed(2)} €</span></div>
              </>
            )}
          </div>
        </div>
