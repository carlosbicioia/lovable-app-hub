...
export default function Purchases() {
  ...
  const monthlyData = useMemo(() => {
    ...
    return Array.from(map.entries())
      ...
      .slice(-12)
      .map(([month, vals]) => ({ month: format(new Date(month + "-01"), "MMM yy", { locale: es }), ...vals }));
  }, [orders, invoices]);
  ...
  const collaboratorOptions = useMemo(() => {
    const set = new Set<string>();
    for (const sid of serviceOptions) { const s = serviceMap[sid]; if (s?.collaboratorName) set.add(s.collaboratorName); }
    return Array.from(set).sort();
  }, [serviceOptions, serviceMap]);

  const specialtyOptions = useMemo(() => {
    const set = new Set<string>();
    for (const sid of serviceOptions) { const s = serviceMap[sid]; if (s?.specialty) set.add(s.specialty); }
    return Array.from(set).sort();
  }, [serviceOptions, serviceMap]);
  ...
  const filteredOC = useMemo(() => {
    ...
    return orders.filter(
      (o) =>
        (!search || o.id.toLowerCase().includes(q) || o.supplierName.toLowerCase().includes(q) || o.serviceId.toLowerCase().includes(q) || (o.operatorName ?? "").toLowerCase().includes(q)) &&
  ...
  const filteredInv = useMemo(() => {
    ...
    return invoices.filter(
      (i) =>
        (!search || i.invoiceNumber.toLowerCase().includes(q) || i.supplierInvoiceNumber.toLowerCase().includes(q) || i.supplierName.toLowerCase().includes(q)) &&
  ...
  const filteredDN = useMemo(() => {
    ...
    return deliveryNotes.filter(
      (d) =>
        (!search || d.code.toLowerCase().includes(q) || d.supplierName.toLowerCase().includes(q) || d.serviceId.toLowerCase().includes(q) || (d.operatorName ?? "").toLowerCase().includes(q)) &&
  ...
  const handleBulkDelete = async () => {
    if (tab === "oc") {
      for (const o of bulkOC.selectedItems) { await supabase.from("purchase_order_lines").delete().eq("purchase_order_id", o.id); await supabase.from("purchase_orders").delete().eq("id", o.id); }
      bulkOC.clear(); queryClient.invalidateQueries({ queryKey: ["purchase_orders"] }); toast({ title: `${bulkOC.count} orden(es) eliminada(s)` });
    } else if (tab === "albaranes") {
      for (const d of bulkDN.selectedItems) { await supabase.from("delivery_note_lines").delete().eq("delivery_note_id", d.id); await supabase.from("delivery_notes").delete().eq("id", d.id); }
      bulkDN.clear(); queryClient.invalidateQueries({ queryKey: ["delivery_notes"] }); toast({ title: `${bulkDN.count} albarán(es) eliminado(s)` });
    } else {
      for (const i of bulkInv.selectedItems) { await supabase.from("purchase_invoice_lines").delete().eq("invoice_id", i.id); await supabase.from("purchase_invoices").delete().eq("id", i.id); }
      bulkInv.clear(); queryClient.invalidateQueries({ queryKey: ["purchase_invoices"] }); toast({ title: `${bulkInv.count} factura(s) eliminada(s)` });
    }
  };

  const handleBulkExport = () => {
    if (tab === "oc") {
      exportCsv("ordenes_compra.csv", ["ID", "Servicio", "Proveedor", "Operario", "Estado", "Fecha", "Coste"], bulkOC.selectedItems.map((o) => [o.id, o.serviceId, o.supplierName, o.operatorName ?? "", o.status, format(new Date(o.createdAt), "dd/MM/yyyy"), o.totalCost.toString()]));
    } else if (tab === "albaranes") {
      exportCsv("albaranes.csv", ["Código", "Servicio", "Proveedor", "Operario", "Estado", "Fecha", "Coste"], bulkDN.selectedItems.map((d) => [d.code || d.id.slice(0, 8), d.serviceId, d.supplierName, d.operatorName ?? "", d.status, format(new Date(d.createdAt), "dd/MM/yyyy"), d.totalCost.toString()]));
    } else {
      exportCsv("facturas_compra.csv", ["Nº Factura", "Nº Proveedor", "Proveedor", "Fecha", "Estado", "Total"], bulkInv.selectedItems.map((i) => [i.invoiceNumber, i.supplierInvoiceNumber, i.supplierName, i.invoiceDate ? format(new Date(i.invoiceDate), "dd/MM/yyyy") : "", i.status, i.total.toString()]));
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-display font-bold text-foreground">Compras</h1></div>
        <div className="flex items-center gap-2">
          {tab === "oc" && (<Button onClick={() => router.push("/compras/nueva")}><Plus className="w-4 h-4 mr-2" /> Nueva OC</Button>)}
          {tab === "albaranes" && (<Button onClick={() => router.push("/compras/albaran/nuevo")}><Plus className="w-4 h-4 mr-2" /> Nuevo albarán</Button>)}
          {tab === "facturas" && (<Button onClick={() => router.push("/compras/factura/nueva")}><Plus className="w-4 h-4 mr-2" /> Nueva factura</Button>)}
        </div>
      </div>

      <Tabs value={tab} onValueChange={(v) => { setTab(v); setFilterStatus("all"); bulkOC.clear(); bulkDN.clear(); bulkInv.clear(); }}>
        <TabsList className="mb-4">
          <TabsTrigger value="oc" className="gap-1.5"><ShoppingCart className="w-3.5 h-3.5" /> Órdenes<Badge variant="secondary" className="text-[10px] h-5 ml-1">{filteredOC.length}</Badge></TabsTrigger>
          <TabsTrigger value="albaranes" className="gap-1.5"><Truck className="w-3.5 h-3.5" /> Albaranes<Badge variant="secondary" className="text-[10px] h-5 ml-1">{filteredDN.length}</Badge></TabsTrigger>
          <TabsTrigger value="facturas" className="gap-1.5"><FileText className="w-3.5 h-3.5" /> Facturas<Badge variant="secondary" className="text-[10px] h-5 ml-1">{filteredInv.length}</Badge></TabsTrigger>
        </TabsList>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <SearchableSelect options={[{ value: "all", label: "Todos los proveedores" }, ...supplierOptions.map((s) => ({ value: s, label: s }))]} value={filterSupplier} onValueChange={setFilterSupplier} placeholder="Proveedor" searchPlaceholder="Buscar proveedor..." emptyText="Sin resultados" className="w-[200px] h-9 text-sm" />
          <Select value={filterStatus} onValueChange={setFilterStatus}>
            ...
            <SelectContent><SelectItem value="all">Todos</SelectItem>{statusOptionsForTab.map((s) => (<SelectItem key={s} value={s}>{statusLabelMap[s] || s}</SelectItem>))}</SelectContent>
          </Select>
          {tab !== "facturas" && (
            <SearchableSelect options={[{ value: "all", label: "Todos los servicios" }, ...serviceOptions.map((s) => ({ value: s, label: s }))]} value={filterService} onValueChange={setFilterService} placeholder="Servicio" searchPlaceholder="Buscar servicio..." emptyText="Sin resultados" className="w-[200px] h-9 text-sm" />
          )}
          <DatePresetSelect dateFrom={dateFrom} dateTo={dateTo} onDateChange={(from, to) => { setDateFrom(from); setDateTo(to); }} />
        </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredOC.length === 0 ? (
                    ...
                    return (
                      <tr key={o.id} className={cn("border-b border-border last:border-0 hover:bg-muted/50 transition-colors cursor-pointer", bulkOC.selectedIds.has(o.id) && "bg-primary/5")} onClick={() => router.push(`/compras/${o.id}`)}>
                        ...
                        <td className="px-5 py-3"><span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold border", sc.cls)}>{sc.label}</span></td>
                        ...
                        <td className="px-5 py-3" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-center gap-1">
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDeleteTarget({ type: "oc", id: o.id })}><Trash2 className="w-3.5 h-3.5 text-destructive" /></Button>
                          </div>
                        </td>
                      </tr>
                    );
                  ...
                  {filteredDN.length === 0 ? (
                    ...
                    return (
                      <tr key={dn.id} className={cn("border-b border-border last:border-0 hover:bg-muted/50 transition-colors", bulkDN.selectedIds.has(dn.id) && "bg-primary/5")}>
                        ...
                        <td className="px-5 py-3 text-center">{dn.pdfPath ? (<a href={dn.pdfPath} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline text-xs" onClick={(e) => e.stopPropagation()}>Ver PDF</a>) : "—"}</td>
                        <td className="px-5 py-3" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-center gap-1">
                            <TooltipProvider><Tooltip><TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => {
                                const co = companySettings;
                                generateDocumentPdf({ type: "Albarán", id: dn.code || dn.id.slice(0, 8), date: formatDate(new Date(dn.createdAt), "dd/MM/yyyy"), company: { companyName: co?.company_name || "UrbanGO", logoUrl: co?.logo_url, taxId: co?.tax_id, address: co?.address, documentFooter: co?.document_footer }, recipientName: dn.supplierName, infoFields: [{ label: "Servicio", value: dn.serviceId }, { label: "Operario", value: dn.operatorName || "—" }, { label: "Estado", value: dn.status }], lines: dn.lines.map((l) => ({ description: l.articleName + (l.description ? ` — ${l.description}` : ""), units: l.units, unitPrice: l.costPrice, total: l.units * l.costPrice })), subtotal: dn.totalCost, taxBreakdown: [], total: dn.totalCost, notes: dn.notes || undefined });
                              }}><Download className="w-3.5 h-3.5 text-primary" /></Button>
                            </TooltipTrigger><TooltipContent>Generar PDF</TooltipContent></Tooltip></TooltipProvider>
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDeleteTarget({ type: "dn", id: dn.id })}><Trash2 className="w-3.5 h-3.5 text-destructive" /></Button>
                          </div>
                        </td>
                      </tr>
                    );
                  ...
                  {filteredInv.length === 0 ? (
                    ...
                    return (
                      <tr key={inv.id} className={cn("border-b border-border last:border-0 hover:bg-muted/50 transition-colors cursor-pointer", bulkInv.selectedIds.has(inv.id) && "bg-primary/5")}>
                        ...
                        <td className="px-5 py-3 text-muted-foreground text-xs">{inv.invoiceDate ? format(new Date(inv.invoiceDate), "dd MMM yyyy", { locale: es }) : "—"}</td>
                        <td className="px-5 py-3"><span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold border", sc.cls)}>{sc.label}</span></td>
                        ...
                        <td className="px-5 py-3 text-center">{inv.pdfPath ? (<a href={inv.pdfPath} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline text-xs" onClick={(e) => e.stopPropagation()}>Ver PDF</a>) : "—"}</td>
                        <td className="px-5 py-3" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-center gap-1">
                            <TooltipProvider><Tooltip><TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => {
                                ...
                                generateDocumentPdf({ type: "Factura de Compra", id: inv.invoiceNumber || inv.id.slice(0, 8), date: inv.invoiceDate ? formatDate(new Date(inv.invoiceDate), "dd/MM/yyyy") : formatDate(new Date(inv.createdAt), "dd/MM/yyyy"), company: { companyName: co?.company_name || "UrbanGO", logoUrl: co?.logo_url, taxId: co?.tax_id, address: co?.address, documentFooter: co?.document_footer }, recipientName: inv.supplierName, infoFields: [...(inv.dueDate ? [{ label: "Vencimiento", value: formatDate(new Date(inv.dueDate), "dd/MM/yyyy") }] : []), { label: "Estado", value: inv.status }], lines: invLines.map((l) => ({ description: l.description, units: l.units, unitPrice: l.unitPrice, taxRate: l.taxRate, total: l.total })), subtotal: inv.subtotal, taxBreakdown: Object.entries(taxByRate).map(([rate, amount]) => ({ rate: Number(rate), amount })), total: inv.total, notes: inv.notes || undefined });
                              }}><Download className="w-3.5 h-3.5 text-primary" /></Button>
                            </TooltipTrigger><TooltipContent>Generar PDF</TooltipContent></Tooltip></TooltipProvider>
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDeleteTarget({ type: "invoice", id: inv.id })}><Trash2 className="w-3.5 h-3.5 text-destructive" /></Button>
                          </div>
                        </td>
                      </tr>
                    );
