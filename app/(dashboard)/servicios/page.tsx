    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button onClick={() => router.push("/servicios/nuevo")} size="sm">
      ...
      <div className="md:hidden space-y-2">
        {filtered.length === 0 ? (
          ...
          return (
            <div
              key={s.id}
              onClick={() => { if (isBillingTab) bulk.toggle(s.id); else router.push(`/servicios/${s.id}`); }}
              className={cn(
                      ...
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs font-semibold text-foreground hover:text-primary hover:underline"
                            onClick={(e) => { if (isBillingTab) { e.stopPropagation(); router.push(`/servicios/${s.id}`); } }}
                          >{s.id}</span>
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-muted text-muted-foreground">{s.origin}</span>
