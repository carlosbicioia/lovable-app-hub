...
export default function PurchaseCreate() {
  ...
  const handleSubmit = () => {
    if (!nextId || !serviceId) { toast.error("Selecciona un servicio"); return; }
    if (!supplierName) { toast.error("Selecciona un proveedor"); return; }
    if (lines.some((l) => !l.articleName.trim())) { toast.error("Todos los artículos deben tener nombre"); return; }
    if (lines.some((l) => l.units <= 0)) { toast.error("Las unidades deben ser mayor que 0"); return; }
    if (lines.some((l) => l.costPrice <= 0)) { toast.error("El coste debe ser mayor que 0 en todas las líneas"); return; }
    createOrder.mutate(
      { id: nextId, serviceId, supplierName, operatorId: operatorId || null, operatorName: selectedOp?.name ?? null, notes, lines },
      { onSuccess: () => router.push("/compras") }
    );
  };

  if (loadingId) return <div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div>;
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="space-y-1.5">
              ...
              <SearchableSelect value={serviceId} onValueChange={setServiceId} placeholder="Buscar servicio…" searchPlaceholder="Nº servicio, cliente…" emptyText="Sin servicios"
                options={services.map((s) => ({ value: s.id, label: `${s.id} - ${s.clientName}`, subtitle: s.address ?? undefined, searchText: `${s.description ?? ""} ${s.address ?? ""}` }))} />
            </div>
            <div className="space-y-1.5">
              <Label>Proveedor *</Label>
              <SearchableSelect value={supplierName} onValueChange={setSupplierName} placeholder="Buscar proveedor…" searchPlaceholder="Nombre, CIF…" emptyText="Sin proveedores"
                options={suppliers.filter((s) => s.active).map((s) => ({ value: s.name, label: s.name, subtitle: s.city || undefined, searchText: `${s.taxId} ${s.contactPerson}` }))} />
            </div>
            <div className="space-y-1.5">
              <Label>Operario</Label>
              <SearchableSelect value={operatorId} onValueChange={setOperatorId} placeholder="Asignar operario…" searchPlaceholder="Nombre del operario…" emptyText="Sin operarios"
                options={operators.map((o) => ({ value: o.id, label: o.name, subtitle: `${o.specialty} · NPS ${o.npsMean}` }))} />
            </div>
            <div className="space-y-1.5">
