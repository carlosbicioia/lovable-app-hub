...
export default function PurchaseDetailPage({ params }: { params: { id: string } }) {
  ...
  const startEdit = useCallback(() => {
    if (!order) return;
    setEditLines(order.lines.map((l) => ({ articleName: l.articleName, description: l.description, units: l.units, costPrice: l.costPrice, taxRate: l.taxRate })));
    setEditing(true);
  }, [order]);
  ...
  const saveLines = () => {
    if (!order) return;
    updateLines.mutate({ orderId: order.id, lines: editLines }, { onSuccess: () => setEditing(false) });
  };
  ...
  const addEditLine = () => setEditLines((prev) => [...prev, { articleName: "", description: "", units: 1, costPrice: 0, taxRate: 21 }]);
  const removeEditLine = (idx: number) => setEditLines((prev) => (prev.length > 1 ? prev.filter((_, i) => i !== idx) : prev));
  ...
  const computeTotals = useCallback((lines: { units: number; costPrice: number; taxRate: number }[]) => {
    ...
    lines.forEach((l) => { const base = l.units * l.costPrice; taxByRate[l.taxRate] = (taxByRate[l.taxRate] || 0) + base * (l.taxRate / 100); });
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-display font-bold text-foreground flex items-center gap-2"><ShoppingCart className="w-5 h-5 text-primary" /> {order.id}</h1>
        ...
        <Button variant="outline" onClick={generatePdf} disabled={generatingPdf}>
          {generatingPdf ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Download className="w-4 h-4 mr-2" />} Generar PDF
        </Button>
      </div>
      ...
      <div className="flex items-center gap-0">
        {statusFlow.map((s, i) => {
          ...
          return (
            <button key={s.status} onClick={() => { if (s.status !== order.status) updateStatus.mutate({ id: order.id, status: s.status }); }}
              disabled={updateStatus.isPending}
              className={cn("flex items-center gap-1.5 px-4 py-2 text-xs font-semibold border transition-colors relative",
                i === 0 && "rounded-l-lg", i === statusFlow.length - 1 && "rounded-r-lg",
                isCurrent ? statusColors[s.status] + " ring-2 ring-primary/30 z-10"
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            <div><p className="text-xs text-muted-foreground">Proveedor</p><p className="font-medium text-foreground">{order.supplierName}</p></div>
            <div><p className="text-xs text-muted-foreground">Operario</p><p className="font-medium text-foreground">{order.operatorName ?? "—"}</p></div>
            <div><p className="text-xs text-muted-foreground">Fecha creación</p><p className="font-medium text-foreground">{format(new Date(order.createdAt), "dd MMM yyyy HH:mm", { locale: es })}</p></div>
            <div>
              <p className="text-xs text-muted-foreground">Servicio</p>
              <p className="font-medium text-foreground">
                <button onClick={() => router.push(`/servicios/${order.serviceId}`)} className="text-primary hover:underline">{order.serviceId}</button>
              </p>
            </div>
          </div>
          {service && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm mt-4 pt-4 border-t border-border">
              <div><p className="text-xs text-muted-foreground">Tipología servicio</p><p className="font-medium text-foreground flex items-center gap-1.5"><Wrench className="w-3.5 h-3.5 text-muted-foreground" />{service.serviceType === "Presupuesto" ? "Con presupuesto" : "Reparación directa"}</p></div>
              <div><p className="text-xs text-muted-foreground">Especialidad</p><p className="font-medium text-foreground">{service.specialty}</p></div>
              <div>
                ...
                )}>{service.status.replace(/_/g, " ")}</span>
              </div>
              <div><p className="text-xs text-muted-foreground">Cliente</p><p className="font-medium text-foreground">{service.clientName}</p></div>
            </div>
          )}
          ...
          {order.pdfPath && (
            <div className="mt-4 flex items-center gap-2">
              ...
              <a href={order.pdfPath} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline text-sm">Ver PDF generado</a>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between space-y-0">
          <CardTitle className="text-base flex items-center gap-2"><Package className="w-4 h-4 text-muted-foreground" /> Líneas</CardTitle>
          {order.status === "Borrador" && (
            !editing ? (
              <Button variant="outline" size="sm" onClick={startEdit}><Pencil className="w-3.5 h-3.5 mr-1" /> Editar</Button>
            ) : (
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={addEditLine}><Plus className="w-3.5 h-3.5 mr-1" /> Añadir</Button>
                <Button variant="outline" size="sm" onClick={cancelEdit}><X className="w-3.5 h-3.5 mr-1" /> Cancelar</Button>
                <Button size="sm" onClick={saveLines} disabled={updateLines.isPending}>
                  {updateLines.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-1" /> : <Save className="w-3.5 h-3.5 mr-1" />} Guardar
                </Button>
              </div>
            )
          )}
        ...
        <CardContent>
            ...
            <div className="space-y-3">
              {editLines.map((l, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-end">
                  <div className="col-span-3 space-y-1">{idx === 0 && <Label className="text-xs">Artículo</Label>}<Input value={l.articleName} onChange={(e) => updateEditLine(idx, { articleName: e.target.value })} placeholder="Nombre" /></div>
                  <div className="col-span-3 space-y-1">{idx === 0 && <Label className="text-xs">Descripción</Label>}<Input value={l.description} onChange={(e) => updateEditLine(idx, { description: e.target.value })} placeholder="Desc." /></div>
                  <div className="col-span-1 space-y-1">{idx === 0 && <Label className="text-xs">Uds.</Label>}<Input type="number" value={l.units} onChange={(e) => updateEditLine(idx, { units: Number(e.target.value) || 0 })} /></div>
                  <div className="col-span-2 space-y-1">{idx === 0 && <Label className="text-xs">Precio</Label>}<Input type="number" step="0.01" value={l.costPrice} onChange={(e) => updateEditLine(idx, { costPrice: Number(e.target.value) || 0 })} /></div>
                  <div className="col-span-2 space-y-1">
                    ...
                    <Select value={String(l.taxRate)} onValueChange={(v) => updateEditLine(idx, { taxRate: Number(v) })}>
                      ...
                      <SelectContent>
                        {taxTypes.length > 0 ? taxTypes.map((tt) => <SelectItem key={tt.id} value={String(tt.rate)}>{tt.name} ({tt.rate}%)</SelectItem>) : [0, 4, 10, 21].map((r) => <SelectItem key={r} value={String(r)}>{r}%</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="col-span-1"><Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => removeEditLine(idx)} disabled={editLines.length === 1}><Trash2 className="w-4 h-4 text-destructive" /></Button></div>
                </div>
              ))}
            </div>
          ...
          <div className="border-t border-border pt-4 mt-4 space-y-1 text-sm text-right">
            <p className="text-muted-foreground">Base imponible: <span className="font-medium text-foreground">€{totals.subtotal.toLocaleString("es-ES", { minimumFractionDigits: 2 })}</span></p>
            {Object.entries(totals.taxByRate).map(([rate, amount]) => (
              <p key={rate} className="text-muted-foreground">IVA {rate}%: <span className="font-medium text-foreground">€{(amount as number).toLocaleString("es-ES", { minimumFractionDigits: 2 })}</span></p>
            ))}
            <p className="text-base font-semibold text-foreground pt-1 border-t border-border">Total: €{totals.total.toLocaleString("es-ES", { minimumFractionDigits: 2 })}</p>
          </div>
        </CardContent>
      </Card>
    </div>
