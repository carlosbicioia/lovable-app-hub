              ...
              <TableBody>
                {filtered.map((s) => (
                  <TableRow key={s.id} className="cursor-pointer hover:bg-muted/50" onClick={() => openEdit(s)}>
                    ...
                    <TableCell className="text-sm">{s.paymentTerms} · {s.dueDays}d</TableCell>
                    <TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
            <div className="space-y-1.5"><Label>Nombre comercial *</Label><Input value={form.name} onChange={(e) => updateField("name", e.target.value)} placeholder="Ej: Saltoki" /></div>
            <div className="space-y-1.5"><Label>CIF / NIF</Label><Input value={form.taxId} onChange={(e) => updateField("taxId", e.target.value)} placeholder="B12345678" /></div>
            <div className="space-y-1.5"><Label>Persona de contacto</Label><Input value={form.contactPerson} onChange={(e) => updateField("contactPerson", e.target.value)} /></div>
            ...
            <div className="col-span-2 grid grid-cols-12 gap-3">
              <div className="col-span-6 space-y-1.5"><Label>Calle</Label><Input value={form.address} onChange={(e) => updateField("address", e.target.value)} placeholder="Nombre de la calle" /></div>
              <div className="col-span-2 space-y-1.5"><Label>Número</Label><Input value={form.streetNumber} onChange={(e) => updateField("streetNumber", e.target.value)} placeholder="Nº" /></div>
              <div className="col-span-2 space-y-1.5"><Label>Piso</Label><Input value={form.floor} onChange={(e) => updateField("floor", e.target.value)} placeholder="1ºA" /></div>
              <div className="col-span-2 space-y-1.5"><Label>Adicional</Label><Input value={form.addressExtra} onChange={(e) => updateField("addressExtra", e.target.value)} placeholder="Esc, puerta..." /></div>
            </div>
            ...
            <div className="space-y-1.5"><Label>IBAN</Label><Input value={form.iban} onChange={(e) => updateField("iban", e.target.value)} placeholder="ES00 0000 0000 0000 0000 0000" /></div>
            ...
            <div className="space-y-1.5"><Label>Días de vencimiento</Label><Input type="number" min={0} value={form.dueDays} onChange={(e) => updateField("dueDays", Number(e.target.value))} /></div>
            <div className="flex items-center gap-3"><Switch checked={form.active} onCheckedChange={(v) => updateField("active", v)} /><Label>Activo</Label></div>
            <div className="col-span-full space-y-1.5"><Label>Notas</Label><Textarea value={form.notes} onChange={(e) => updateField("notes", e.target.value)} rows={3} placeholder="Observaciones..." /></div>
          </div>
