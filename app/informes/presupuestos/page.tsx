...
export default function BudgetsReport() {
  ...
  const stats = useMemo(() => {
    ...
    const totalAmount = budgets.reduce((acc, b) => { const svc = services.find((s) => s.id === b.serviceId); return acc + (svc?.budgetTotal || 0); }, 0);
    const approvedAmount = budgets.filter((b) => ["Aprobado", "Finalizado", "Pte_Facturación"].includes(b.status)).reduce((acc, b) => { const svc = services.find((s) => s.id === b.serviceId); return acc + (svc?.budgetTotal || 0); }, 0);
    ...
    const monthly = Object.entries(monthMap).sort((a, b) => a[0].localeCompare(b[0])).map(([name, v]) => ({ name, presupuestos: v.count, importe: Math.round(v.amount) }));
    ...
    budgets.forEach((b) => { const key = b.collaboratorName || "Sin colaborador"; collabMap[key] = (collabMap[key] || 0) + 1; });
